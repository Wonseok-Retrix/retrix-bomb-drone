...
> [!WARNING]
> 이 서비스는 실제 MAVLink 명령과 드로퍼 출력을 활성화합니다. 지상 점검이 끝난 기체에서만 등록하세요. 프로그램은 시동, CH8 허용 스위치와 중립 스틱을 확인한 뒤 RC override를 허용하며 비행 모드는 검사하지 않습니다. 활성 중에는 RC1~RC4 스틱이 무시되므로 수동 복귀 시 CH8을 LOW로 내리세요. 드로퍼는 미시동 상태에서도 CH8이 HIGH이고 영상 조건이 맞으면 작동합니다.
