...
* **세로 회색 영역**: 데드밴드 (Deadband, 목표물이 이 영역 내에 있으면 미세 이동 명령을 0으로 억제)
...
> [!WARNING]
> 이 서비스는 실제 MAVLink 명령과 드로퍼 출력을 활성화합니다. 지상 점검이 끝난 기체에서만 등록하세요. 프로그램은 시동과 CH8 허용 스위치를 확인한 뒤 RC override를 즉시 시작하며 비행 모드와 RC1~RC3 시작 위치는 검사하지 않습니다. 활성 중에는 RC1~RC3 스틱이 무시되고 yaw(RC4)는 계속 조종할 수 있습니다. 수동 복귀 시 CH8을 LOW로 내리세요. 드로퍼는 미시동 상태에서도 CH8이 HIGH이고 영상 조건이 맞으면 작동합니다.
